...
class DocsExtractPipeline:
    ...
    def load_docs(self, docs_collection_path: str | None = None):
        """
        Retrieves and iterates over documents from a given source, using either a specified path or a preset configuration, and automatically selects the most suitable loader based on the document type or structure.
...
class DocsTransformPipeline:
    ...
    def transform(self, batch_size: int | None = None) -> Iterable[list[Document]]:
        """
        Processes input documents in batches, sequentially applying the pipeline's transformations to each group.
...
class DocsLoadPipeline:
    ...
    def __init__(self, docs_generator: Iterable[list[Document]]):
        """
        Sets up the pipeline with an input source for document batches and applies standard configuration settings.

        Args:
            docs_generator: An iterable that yields lists of Document objects.
    ...
    def load(
        ...
        """
        Transfers document data efficiently in batches to a persistent vector storage system, enabling scalable and organized handling of large collections.
