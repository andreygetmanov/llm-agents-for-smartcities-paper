...
def merge_collections(
    ...
    """
    Combine the contents of two specified data collections into a single collection. If a new collection name is provided, merges are performed into this new collection; if not, unique items from the second collection are added to the first. Ensures no duplicate documents in the merged result. Exceptions are raised if database or network access fails.
...
def delete_repeats(collection: Chroma) -> None:
    """
    Eliminate repeated entries within the specified dataset to ensure each document appears only once.
...
def get_all_docs_names(collection: Chroma) -> set[str]:
    """
    Retrieve the unique names of all documents available within the specified collection.
...
def insert_documents(collection: Chroma, docs: Iterable[Document]):
    """
    Add documents to the collection only if their names are not already present, ensuring no duplicates based on document source names.
